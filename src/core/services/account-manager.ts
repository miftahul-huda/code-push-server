...
class AccountManager {
    ...
    login(account: string, password: string) {
        ...
        return Users.findOne({ where })
            .then((users) => {
                if (_.isEmpty(users)) {
                    throw new AppError('The email address or password you entered is incorrect');
                }
                return users;
            })
            .then((users) => {
                if (tryLoginTimes > 0) {
                    ...
                    return redisClient.get(loginKey).then((loginErrorTimes) => {
                        if (Number(loginErrorTimes) > tryLoginTimes) {
                            throw new AppError(`Your account has been locked due to too many failed login attempts.`);
                        }
                        return users;
                    });
                }
    ...
    sendRegisterCode(email: string) {
        ...
        return Users.findOne({ where: { email } })
            .then((u) => {
                if (u) {
                    throw new AppError(`"${email}" You have already registered, please use a different email address to register.`);
                }
            })
            .then(() => {
                // 将token临时存储到redis
            ...
            .then(() => {
                ...
                return redisClient.get(registerKey).then((storageToken) => {
                    if (_.isEmpty(storageToken)) {
                        throw new AppError(` The verification code has expired, please get a new one.`);
                    }
                    if (!_.eq(token, storageToken)) {
                        ...
                        throw new AppError(`The verification code you entered is incorrect, please try again.`);
                    }
                    return storageToken;
                });
            });
    ...
    changePassword(uid: number, oldPassword: string, newPassword: string) {
        if (!_.isString(newPassword) || newPassword.length < 6) {
            return Promise.reject(new AppError('Please enter a new password that is 6 to 20 characters long.'));
        }
        return Users.findOne({ where: { id: uid } })
            ...
            .then((u) => {
                ...
                if (!isEq) {
                    throw new AppError(`The old password you entered is incorrect, please try again.`);
                }
